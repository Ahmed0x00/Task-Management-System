                ...
                try {
                    ...
                    if (!date.isBefore(startDate) && !date.isAfter(endDate)) {
                        String displayText = loggedInUser.getRole() == User.Role.LEADER
                                ? task.getTitle() + " (" + UserUtil.getEmployeeName(task.getAssignedEmployeeId()) + ")"
                                : task.getTitle();
    ...
    private void showPhaseDetails(TaskPhase phase, List<Task> allTasks) {
        ...
        if (phaseTasks.isEmpty()) {
            ...
            for (Task task : phaseTasks) {
                ...
                if (UserUtil.getLoggedInUser().getRole() == User.Role.LEADER) {
                    taskList.append(" (").append(UserUtil.getEmployeeName(task.getAssignedEmployeeId())).append(")");
                }
                taskList.append("\n");
            }
        }
