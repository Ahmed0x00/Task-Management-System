...
import com.example.taskmanagement.util.ProjectUtil;
import com.example.taskmanagement.util.TaskPhaseUtil;
...
public class ManageTasksController {
    ...
    private void loadTasks() {
        ...
        if (loggedInUser.getRole() == User.Role.EMPLOYEE) {
            ...
            tasks = TaskUtil.getTasksByEmployeeId(employeeId);
        } else {
    ...
    private VBox createTaskCard(Task task) {
        ...
        if (task.getAssignedEmployeeId() != null) {
            Employee employee = EmployeeUtil.getById(task.getAssignedEmployeeId());
        ...
        if (task.getProjectId() != null) {
            Project project = ProjectUtil.getById(task.getProjectId());
        ...
        if (task.getPhase() != null) {
            TaskPhase phase = TaskPhaseUtil.getById(task.getPhase());
        ...
        if (task.getCreatorUserId() != null) {
            User creator = UserUtil.getById(task.getCreatorUserId());
        ...
        if (!"Completed".equalsIgnoreCase(phaseName)) {
            if (loggedInUser.getRole() == User.Role.EMPLOYEE && task.getAssignedEmployeeId() != null && task.getAssignedEmployeeId().equals(loggedInUser.getEmployeeId())) {
                TaskPhase phase = task.getPhase() != null ? TaskPhaseUtil.getById(task.getPhase()) : null;
                ...
                actionButton.setOnAction(e -> {
                    if (isUnderWork) {
                        ...
                        TaskPhase completedPhase = TaskPhaseUtil.findByName("Completed");
                        if (completedPhase != null) {
                            task.setPhase(completedPhase.getId());
                            TaskUtil.updateTask(task);
                            // Update task log
                            TaskLog log = TaskLogUtil.getByTaskIdAndEmployeeId(task.getId(), loggedInUser.getEmployeeId());
                            if (log != null) {
                        ...
                        TaskPhase underWorkPhase = TaskPhaseUtil.findByName("Under Work");
                ...
                deleteBtn.setOnAction(e -> {
                    TaskUtil.deleteTaskById(task.getId());
                    loadTasks();
                });
    ...
    public void onAddTask() {
        TaskPhase defaultPhase = TaskPhaseUtil.findByName("Pending");
    ...
    private void showTaskDialog(Task task, boolean editing) {
        ...
        List<Project> projects = ProjectUtil.getAllProjects();
        ...
        List<TaskPhase> phases = TaskPhaseUtil.getAllTaskPhases();
        ...
        dialog.showAndWait().ifPresent(result -> {
            if (editing) {
                TaskUtil.updateTask(result);
            } else {
                TaskUtil.addTask(result);
            }
            loadTasks();
        });
    }
