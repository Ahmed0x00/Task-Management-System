...
import com.example.taskmanagement.util.EmployeeTypeUtil;
...
public class ManageEmployeesController {
    ...
    private VBox createEmployeeCard(Employee employee) {
        ...
        EmployeeType type = EmployeeTypeUtil.getById(employee.getEmployeeTypeId());
        ...
        deleteBtn.setOnAction(e -> {
            EmployeeUtil.deleteEmployeeById(employee.getId());
            loadEmployees();
        });
    ...
    public void onAddEmployee() {
        ...
        userDialog.setResultConverter(btn -> {
            if (btn == ButtonType.OK) {
                ...
                if (UserUtil.getAllUsers().stream().anyMatch(u -> u.getUsername().equalsIgnoreCase(username))) {
    ...
    private void showEmployeeDialog(Employee employee, boolean editing) {
        ...
        typeCombo.getItems().addAll(EmployeeTypeUtil.getAllEmployeeTypes());
        ...
        // Set initial value
        typeCombo.setValue(employee.getEmployeeTypeId() != 0 ? EmployeeTypeUtil.getById(employee.getEmployeeTypeId()) : null);
        ...
        dialog.showAndWait().ifPresent(result -> {
            if (editing) {
                EmployeeUtil.updateEmployee(result);
            } else {
                EmployeeUtil.addEmployee(result);
            }
            loadEmployees();
        });
    }
