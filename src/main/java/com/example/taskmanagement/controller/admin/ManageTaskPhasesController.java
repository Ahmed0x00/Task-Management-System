...
public class ManageTaskPhasesController {
    ...
    private void loadTaskPhases() {
        ...
        List<TaskPhase> taskPhases = TaskPhaseUtil.getAllTaskPhases();
    ...
    private VBox createTaskPhaseCard(TaskPhase taskPhase) {
        ...
        deleteBtn.setOnAction(e -> {
            TaskPhaseUtil.deleteTaskPhaseById(taskPhase.getId());
            loadTaskPhases();
        });
    ...
    private void showTaskPhaseDialog(TaskPhase taskPhase, boolean editing) {
        ...
        dialog.showAndWait().ifPresent(result -> {
            if (editing) {
                TaskPhaseUtil.updateTaskPhase(result);
            } else {
                TaskPhaseUtil.addTaskPhase(result);
            }
            loadTaskPhases();
        });
    }
